...
public class Reusable_Actions_Loggers_POM {
    ...
    public static void clickByIndex(WebDriver driver, WebElement xpath, int indexNumber, ExtentTest logger, String elementName){
        ...
        try{
            wait.until(ExpectedConditions.visibilityOfAllElements((xpath))).get(indexNumber).click();
        } catch (Exception e) {
    ...
    public static void SelectMethod(WebDriver driver, WebElement xpath, String userData, ExtentTest logger, String elementName){
        WebDriverWait wait = new WebDriverWait(driver,15);
        System.out.println("Selecting element " + elementName);
        logger.log(LogStatus.INFO,"Selecting " + elementName);
        try{
    ...
    public static void multipleClick (WebDriver driver, WebElement xpath, Integer number,ExtentTest logger, String elementName){
        WebDriverWait wait = new WebDriverWait(driver,15);
        System.out.println("Clicking on element " + elementName);
        logger.log(LogStatus.INFO,"Clicking on element " + elementName);
        try {
            //driver.findElements(By.xpath(xpath)).get(n).click();
            wait.until(ExpectedConditions.visibilityOfAllElements((xpath))).get(number).click();
        } catch (Exception e) {
            System.out.println("Unable to click on " + elementName + " Error:" + e);
            logger.log(LogStatus.FAIL,"Unable to click on " + elementName + " Error:" + e);
        }
