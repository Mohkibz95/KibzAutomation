...
public class Reusable_Actions_Loggers {
    ...
    public static void selectByVisibleText(WebDriver driver, String xpath, String userData, ExtentTest logger, String elementName){
        ...
        try{
            ...
            dDown.selectByVisibleText(userData);
        } catch (Exception e) {
    ...
    //Click on pop up
    public static void clickOnPopup(WebDriver driver, String xpath, ExtentTest logger, String elementName){
        WebDriverWait wait = new WebDriverWait(driver,15);
    ...
    public static void submitMethod(WebDriver driver, String xpath, ExtentTest logger, String elementName){
        ...
        try {
            ...
            logger.log(LogStatus.FAIL,"Unable to submit " + elementName + " Error:" + e);
            getScreenShot(driver,elementName,logger);
        }
    ...
    //working multiple element to hover over
    public static void multipleMouseHover(WebDriver driver, String xpath, Integer number, ExtentTest logger, String elementName) throws InterruptedException {
        //declare wait
        Thread.sleep(3000);
        ...
        logger.log(LogStatus.INFO, "Hovering on element " + elementName);
        try {
            WebElement sendTab = driver.findElements(By.xpath(xpath)).get(number);
            //WebElement sendTab = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath(xpath))).get(number);
            mouseAction.moveToElement(sendTab).perform();
            logger.log(LogStatus.PASS, "Successfully hover to element " + elementName);
        } catch (Exception e) {
            System.out.println("Unable to hover on " + elementName + " Error:" + e);
        }
    }//End of MultipleMouseHover
